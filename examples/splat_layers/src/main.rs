//! Splat layers example — visualize terrain material layer blending.
//!
//! Uses `TerrainDebugColorMode::ByLayerDominance` to color each vertex by its
//! dominant material layer. This reveals how the five layers (water, meadow,
//! dirt, rock, snow) blend based on height ranges, slope ranges, and
//! weight-map channels.
...
fn main() {
    ...
    app.add_plugins(DefaultPlugins.set(WindowPlugin {
        primary_window: Some(Window {
            title: "Terrain — Splat Layers".into(),
...
fn setup(mut commands: Commands, mut debug: ResMut<TerrainDebugConfig>) {
    // Wider visual radius to see more layer transitions at once.
    ...
    // Debug: color by dominant layer so the splatmap is directly visible.
